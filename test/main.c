...
char *get_string_from_file(const char *filename) {
    ...
    char c;
    while (SDL_RWread(rwops, &c, 1, 1) > 0) { ++size; }
...
void loop_drawSomeText() {
...
#else
    FC_LoadFont(font, renderer, "fonts/FreeSans.ttf", 20, FC_MakeColor(0, 0, 0, 255),
                TTF_STYLE_NORMAL);
    FC_LoadFont(font2, renderer, "fonts/FreeSans.ttf", 18, FC_MakeColor(0, 200, 0, 255),
                TTF_STYLE_NORMAL);
    FC_LoadFont(font3, renderer, "fonts/FreeSans.ttf", 22, FC_MakeColor(0, 0, 200, 255),
                TTF_STYLE_NORMAL);
#endif
    ...
    while (!done) {
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT)
                ...
                if (event.key.keysym.sym == SDLK_BACKSPACE) {
                    ...
                    --input_position;
                    if (input_position < 0) input_position = 0;
                    ...
                    if (input_position >= len) input_position = len;
                }
                ...
                if (FC_InRect(event.button.x, event.button.y, input_rect)) {
                    input_position = FC_GetPositionFromOffset(
                        font, event.button.x - input_rect.x, event.button.y - input_rect.y,
                        input_rect.w, FC_ALIGN_LEFT, "%s", input_text);
                }
            }
        }
        ...
        if (mode == 0 || mode == 1) {
            ...
            FC_DrawColor(font, target, rightHalf.x, 65,
                         FC_MakeColor(128 + 127 * sin(time), 128 + 127 * sin(time / 2),
                                      128 + 127 * sin(time / 4), 128 + 127 * sin(time / 8)),
                         "Dynamic colored text");
            FC_Draw(font, target, rightHalf.x, 85, "Multi\nline\ntext");

            FC_DrawBox(font, target, input_rect, "%s", input_text);
            draw_rect(FC_MakeRect(input_rect.x, input_rect.y + FC_GetLineHeight(font),
                                  FC_GetWidth(font, "%s", input_text), 2),
                      black);

            FC_Rect input_cursor_pos =
                FC_GetCharacterOffset(font, input_position, input_rect.w, "%s", input_text);
            if (SDL_GetTicks() % 1000 < 500)
                fill_rect(FC_MakeRect(input_rect.x + input_cursor_pos.x,
                                      input_rect.y + input_cursor_pos.y, input_cursor_pos.w,
                                      input_cursor_pos.h),
                          FC_MakeColor(0, 0, 0, 255));

            FC_DrawColumn(font, target, 0, 50, 200,
                          "column align LEFT\nColumn text wraps at the width of the column and has "
                          "no maximum height.");
            FC_DrawColumnAlign(font, target, 100, 250, 200, FC_ALIGN_CENTER,
                               "column align CENTER\nColumn text wraps at the width of the column "
                               "and has no maximum height.");
            FC_DrawColumnAlign(font, target, 200, 450, 200, FC_ALIGN_RIGHT,
                               "column align RIGHT\nColumn text wraps at the width of the column "
                               "and has no maximum height.");
            ...
            FC_DrawBox(font, target, box1a,
                       "box align LEFT\nBox text wraps at the width of the box and is clipped to "
                       "the maximum height.");

            set_clip(box2);
            FC_Rect box2a = {box2.x, box2.y - scroll, box2.w, box2.h + scroll};
            FC_DrawBoxAlign(font, target, box2a, FC_ALIGN_CENTER,
                            "box align CENTER\nBox text wraps at the width of the box and is "
                            "clipped to the maximum height.");

            set_clip(box3);
            FC_Rect box3a = {box3.x, box3.y - scroll, box3.w, box3.h + scroll};
            FC_DrawBoxAlign(font, target, box3a, FC_ALIGN_RIGHT,
                            "box align RIGHT\nBox text wraps at the width of the box and is "
                            "clipped to the maximum height.");
...
int main(int argc, char *argv[]) {
    ...
    window = SDL_CreateWindow("", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h,
                              SDL_WINDOW_SHOWN);
    if (window == NULL) {
