...
static FC_Map *FC_MapCreate(int num_buckets) {
    ...
    for (i = 0; i < num_buckets; ++i) { map->buckets[i] = NULL; }
...
struct FC_Font {
    ...
    // Codepoints are little endian (reversed from UTF-8) so that something like 0x00000005 is ASCII
    // 5 and the map can be indexed by ASCII values
    FC_Map *glyphs;
...
// Private
static FC_GlyphData *FC_PackGlyphData(FC_Font *font, Uint32 codepoint, Uint16 width,
                                      Uint16 maxWidth, Uint16 maxHeight);

static FC_Rect FC_RenderLeft(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
                             const char *text);
static FC_Rect FC_RenderCenter(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
                               const char *text);
static FC_Rect FC_RenderRight(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
                              const char *text);

static_inline SDL_Surface *FC_CreateSurface32(Uint32 width, Uint32 height) {
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
    return SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0xFF000000, 0x00FF0000,
                                0x0000FF00, 0x000000FF);
#else
    return SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, 0x000000FF, 0x0000FF00,
...
FC_Rect FC_DefaultRenderCallback(FC_Image *src, FC_Rect *srcrect, FC_Target *dest, float x, float y,
...
static FC_Rect (*fc_render_callback)(FC_Image *src, FC_Rect *srcrect, FC_Target *dest, float x,
                                     float y, float xscale,
                                     float yscale) = &FC_DefaultRenderCallback;

void FC_SetRenderCallback(FC_Rect (*callback)(FC_Image *src, FC_Rect *srcrect, FC_Target *dest,
...
void FC_SetTabWidth(unsigned int width_in_spaces) { fc_tab_width = width_in_spaces; }
...
static void FC_Init(FC_Font *font) {
    ...
    // Give a little offset for when filtering/mipmaps are used.  Depending on mipmap level, this
    ...
    if (font->glyphs != NULL) FC_MapFree(font->glyphs);
    ...
    if (font->loading_string == NULL) font->loading_string = FC_GetStringASCII();
...
static Uint8 FC_GrowGlyphCache(FC_Font *font) {
    ...
    SDL_Texture *new_level =
        SDL_CreateTexture(font->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
                          font->height * 12, font->height * 12);
#endif
    if (new_level == NULL || !FC_SetGlyphCacheLevel(font, font->glyph_cache_count, new_level)) {
        FC_Log("Error: SDL_FontCache ran out of packing space and could not add another cache "
               "level.\n");
#ifdef FC_USE_SDL_GPU
...
#ifndef FC_USE_SDL_GPU
    {
        ...
        if (prev_target) {
            ...
            if (prev_clip_enabled) prev_clip = get_clip(font->renderer);
        ...
        if (prev_target) {
            if (prev_clip_enabled) set_clip(font->renderer, &prev_clip);
...
Uint8 FC_UploadGlyphCache(FC_Font *font, int cache_level, SDL_Surface *data_surface) {
    ...
    if (!fc_has_render_target_support)
        ...
        if (!old_filter_hint) old_filter_hint = "nearest";
        ...
        new_level = SDL_CreateTexture(renderer, data_surface->format->format,
                                      SDL_TEXTUREACCESS_TARGET, data_surface->w, data_surface->h);
        SDL_SetTextureBlendMode(new_level, SDL_BLENDMODE_BLEND);
...

        {
            ...
            Uint8 prev_clip_enabled;
            float prev_scalex, prev_scaley;
            // only backup if previous target existed (SDL will preserve them for the default
            // target)
            if (prev_target) {
                prev_clip_enabled = has_clip(renderer);
                if (prev_clip_enabled) prev_clip = get_clip(renderer);
            ...
            if (prev_target) {
                if (prev_clip_enabled) set_clip(renderer, &prev_clip);
                if (prev_logicalw && prev_logicalh)
...
static FC_GlyphData *FC_PackGlyphData(FC_Font *font, Uint32 codepoint, Uint16 width,
    ...
    return FC_MapInsert(glyphs, codepoint,
                        FC_MakeGlyphData(last_glyph->cache_level, last_glyph->rect.x,
                                         last_glyph->rect.y, last_glyph->rect.w,
                                         last_glyph->rect.h));
}
...

    {
        ...
        for (; *source_string != '\0'; source_string = U8_next(source_string)) {
            ...
            packed = (FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0), glyph_surf->w,
                                       surfaces[num_surfaces - 1]->w,
                                       surfaces[num_surfaces - 1]->h) != NULL);
            if (!packed) {
                int i = num_surfaces - 1;
                if (num_surfaces >= FC_LOAD_MAX_SURFACES) {
                    // Can't do any more!
                    FC_Log("SDL_FontCache error: Could not create enough cache surfaces to fit all "
                           "of the loading string!\n");
                    SDL_FreeSurface(glyph_surf);
                    break;
                }

                // Upload the current surface to the glyph cache now so we can keep the cache level
                // packing cursor up to date as we go.
                FC_UploadGlyphCache(font, i, surfaces[i]);
                SDL_FreeSurface(surfaces[i]);
#ifndef FC_USE_SDL_GPU
                SDL_SetTextureBlendMode(font->glyph_cache[i], SDL_BLENDMODE_BLEND);
#endif
                // Update the glyph cursor to the new cache level.  We need to do this here because
                // the actual cache lags behind our use of the packing above.
            ...
            if (packed || FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0),
...
#ifdef FC_USE_SDL_GPU
Uint8 FC_LoadFont(FC_Font *font, const char *filename_ttf, Uint32 pointSize, SDL_Color color,
                  int style)
#else
Uint8 FC_LoadFont(FC_Font *font, FC_Target *renderer, const char *filename_ttf, Uint32 pointSize,
...
#ifdef FC_USE_SDL_GPU
Uint8 FC_LoadFont_RW(FC_Font *font, SDL_RWops *file_rwops_ttf, Uint8 own_rwops, Uint32 pointSize,
                     SDL_Color color, int style)
#else
Uint8 FC_LoadFont_RW(FC_Font *font, FC_Target *renderer, SDL_RWops *file_rwops_ttf, Uint8 own_rwops,
...
void FC_ResetFontFromRendererReset(FC_Font *font, SDL_Renderer *renderer, Uint32 evType) {
    ...
    if (evType == SDL_RENDER_TARGETS_RESET) {
        int i;
        for (i = 0; i < font->glyph_cache_count; ++i) SDL_DestroyTexture(font->glyph_cache[i]);
    }
    free(font->glyph_cache);
    ...
    if (owns_ttf) FC_LoadFontFromTTF(font, renderer, ttf, col);
...
unsigned int FC_GetNumCodepoints(FC_Font *font) {
    ...
    for (i = 0; i < glyphs->num_buckets; ++i) {
        FC_MapNode *node;
        for (node = glyphs->buckets[i]; node != NULL; node = node->next) { result++; }
    }
...
static FC_Rect FC_RenderLeft(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
    ...
    for (; *c != '\0'; c++) {
        ...
        if (!FC_GetGlyphData(font, &glyph, codepoint)) {
            codepoint = ' ';
            if (!FC_GetGlyphData(font, &glyph, codepoint)) continue;  // Skip bad characters
        }
        ...
        srcRect = glyph.rect;
#endif
        dstRect = fc_render_callback(FC_GetGlyphCacheLevel(font, glyph.cache_level), &srcRect, dest,
                                     destX, destY, scale.x, scale.y);
...
FC_Rect FC_Draw(FC_Font *font, FC_Target *dest, float x, float y, const char *formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(x, y, 0, 0);
...
static void FC_RenderAlign(FC_Font *font, FC_Target *dest, float x, float y, int width,
                           FC_Scale scale, FC_AlignEnum align, const char *text) {
    switch (align) {
...
static FC_StringList *FC_GetBufferFitToColumn(FC_Font *font, int width, FC_Scale scale,
                                              Uint8 keep_newlines) {
    ...
    for (iter = ls; iter != NULL; iter = iter->next) {
        ...
        if (width > 0 && FC_GetWidth(font, "%s", line) > width) {
            ...
            // Skip the first word for the iterator, so there will always be at least one word per
            // line
...
static void FC_DrawColumnFromBuffer(FC_Font *font, FC_Target *dest, FC_Rect box, int *total_height,
...
FC_Rect FC_DrawBox(FC_Font *font, FC_Target *dest, FC_Rect box, const char *formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(box.x, box.y, 0, 0);
...
FC_Rect FC_DrawBoxAlign(FC_Font *font, FC_Target *dest, FC_Rect box, FC_AlignEnum align,
...
FC_Rect FC_DrawBoxScale(FC_Font *font, FC_Target *dest, FC_Rect box, FC_Scale scale,
                        const char *formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(box.x, box.y, 0, 0);
...
FC_Rect FC_DrawBoxColor(FC_Font *font, FC_Target *dest, FC_Rect box, SDL_Color color,
                        const char *formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(box.x, box.y, 0, 0);
...
FC_Rect FC_DrawBoxEffect(FC_Font *font, FC_Target *dest, FC_Rect box, FC_Effect effect,
                         const char *formatted_text, ...) {
    Uint8 useClip;
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(box.x, box.y, 0, 0);
...
FC_Rect FC_DrawColumn(FC_Font *font, FC_Target *dest, float x, float y, Uint16 width,
                      const char *formatted_text, ...) {
...
static FC_Rect FC_RenderCenter(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
    ...
    for (c = str; *c != '\0';) {
        if (*c == '\n') {
            *c = '\0';
            result = FC_RectUnion(FC_RenderLeft(font, dest,
                                                x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f,
                                                y, scale, str),
                                  result);
            *c = '\n';
            c++;
    ...
    result = FC_RectUnion(
        FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str) / 2.0f, y, scale, str),
        result);
...
static FC_Rect FC_RenderRight(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
    ...
    for (c = str; *c != '\0';) {
        if (*c == '\n') {
            *c = '\0';
            result =
                FC_RectUnion(FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str),
                                           y, scale, str),
                             result);
            *c = '\n';
            c++;
    ...
    result = FC_RectUnion(
        FC_RenderLeft(font, dest, x - scale.x * FC_GetWidth(font, "%s", str), y, scale, str),
        result);
...
FC_Rect FC_DrawScale(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale,
                     const char *formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawAlign(FC_Font *font, FC_Target *dest, float x, float y, FC_AlignEnum align,
                     const char *formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawColor(FC_Font *font, FC_Target *dest, float x, float y, SDL_Color color,
                     const char *formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawEffect(FC_Font *font, FC_Target *dest, float x, float y, FC_Effect effect,
                      const char *formatted_text, ...) {
    if (formatted_text == NULL || font == NULL) return FC_MakeRect(x, y, 0, 0);
...
FC_FilterEnum FC_GetFilterMode(FC_Font *font) {
    if (font == NULL) return FC_FILTER_NEAREST;
...
FC_Rect FC_GetCharacterOffset(FC_Font *font, Uint16 position_index, int column_width,
                              const char *formatted_text, ...) {
    ...
    for (iter = ls; iter != NULL;) {
        ...
        if (next_iter == NULL && !done) result.x = FC_GetWidth(font, "%s", iter->value);
        iter = next_iter;
    }
    FC_StringListFree(ls);

    if (num_lines > 1) { result.y = (num_lines - 1) * FC_GetLineHeight(font); }
...
Uint16 FC_GetColumnHeight(FC_Font *font, Uint16 width, const char *formatted_text, ...) {
    ...
    for (iter = ls; iter != NULL; iter = iter->next) { y += FC_GetLineHeight(font); }
    FC_StringListFree(ls);
...
FC_Rect FC_GetBounds(FC_Font *font, float x, float y, FC_AlignEnum align, FC_Scale scale,
                     const char *formatted_text, ...) {
    FC_Rect result = {x, y, 0, 0};
    ...
    for (iter = ls; iter != NULL; iter = iter->next) {
        ...
        for (line = iter->value; line != NULL && *line != '\0'; line = (char *)U8_next(line)) {
            if (FC_GetGlyphData(font, &glyph_data,
                ...
                if (FC_InRect(
                        x, y,
                        FC_MakeRect(current_x, current_y, glyph_data.rect.w, glyph_data.rect.h))) {
                    done = 1;
                    break;
                }
...
int FC_GetWrappedText(FC_Font *font, char *result, int max_result_size, Uint16 width,
                      const char *formatted_text, ...) {
    FC_StringList *ls, *iter;
...
void FC_SetFilterMode(FC_Font *font, FC_FilterEnum filter) {
    ...
    if (font->filter != filter) {
        ...
        // Update each texture to use this filter mode
        {
            ...
            if (FC_GetFilterMode(font) == FC_FILTER_LINEAR) gpu_filter = GPU_FILTER_LINEAR;
